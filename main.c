 ...
 *
 * /etc/audiod.conf (все ключи необязательны):
...
#define SOCK_PATH   "/run/audiod.sock"
#define LOG_DEFAULT "/var/log/audiod.log"
#define MAX_NODE    64
...
static void config_load(void) {
    FILE *f = fopen("/etc/audiod.conf", "r");
    if (!f) return;
    char line[160];
...
static int is_audio_name(const char *nm) {
...
static int scan_audio(void) {
    ...
    if (fd >= 0) {
        ...
        while ((n = getdents(fd, buf, sizeof(buf))) > 0) {
            ...
            int i;
            for (i = 0; i < count; i++) {
                if (is_audio_name(buf[i].d_name) && found[0] == '\0') {
    ...
    if (strcmp(found, audio_node) == 0) return 0;

    char line[128];
    if (found[0] == '\0') {
        snprintf(line, sizeof(line), "audiod: audio device removed\n");
    } else {
        snprintf(line, sizeof(line), "audiod: audio device present (%s)\n", found);
    }
    log_event(line);
    printf("%s", line);
    strncpy(audio_node, found, sizeof(audio_node) - 1);
    audio_node[sizeof(audio_node) - 1] = '\0';
    return 1;
}
...
static void handle_client(int cl) {
    ...
    if (strcmp(req, "status") == 0) {
        char resp[160];
        if (audio_node[0] == '\0') {
            snprintf(resp, sizeof(resp), "audio=none\n");
        } else {
            snprintf(resp, sizeof(resp), "audio=present path=/dev/%s\n", audio_node);
        }
        send(cl, resp, (uint32_t)strlen(resp), 0);
        log_event("audiod: status requested\n");
        return;
    }
...
int main(int argc, char *argv[]) {
    ...
    printf("audiod: starting\n");
    config_load();

    out_fd = open(log_path, O_WRONLY | O_CREAT | O_APPEND, 0644);
    if (out_fd < 0) {
        printf("audiod: cannot open %s\n", log_path);
    }
    log_event("audiod: starting\n");
    ...
    for (;;) {
        if (srv < 0) {
            ...
            printf("audiod: listening on %s\n", SOCK_PATH);
            log_event("audiod: listening\n");
        }
